...
import cn.wind.xboot.tencent.pojo.StreamIDS;
...
public class RoomServiceImpl implements RoomService {
    ...
    public GetRoomListRsp getRoomList(String userID, String token, GetRoomListReq req, int type) {
        ...
        rsp.setList(roomMgr.getList(req.getCnt(), req.getIndex(),req.getCategory(), true, type, userID));
        return rsp;
    }
    ...
    public CreateRoomRsp createRoom(String userID, String token, CreateRoomReq req, int type) {
        ...
        if (userID == null || token == null || req.getRoomInfo() == null || (req.getRoomID() == null || req.getRoomID().length() == 0) || req.getRoomPic() == null || req.getCityId() == null) {
            rsp.setCode(2);
            rsp.setMessage("请求失败，缺少参数");
            log.error("createRoom失败:缺少参数：" + "userID：" + userID + ",token: " + token + ",roomInfo：" + req.getRoomInfo() + ",type: " + type + ",roomID: "+ req.getRoomID() + ",roomPic: "+ req.getRoomPic() + ",cityId: " + req.getCityId());
            return rsp;
        }
    ...
    public BaseRsp destroyRoom(String userID, String token, DestroyRoomReq req, int type) {
        BaseRsp rsp = new BaseRsp();
        if (userID == null || token == null || req.getRoomID() == null) {
            rsp.setCode(2);
            rsp.setMessage("请求失败，缺少参数");
        ...
        if (!roomMgr.isRoomCreator(req.getRoomID(), userID, type)) {
            rsp.setCode(3);
    ...
    public BaseRsp addPusher(String userID, String token, AddPusherReq req, int type) {
        ...
        if (!roomMgr.isRoomExist(roomID, type)) {
            ...
            // 再创建房间
            roomMgr.creatRoom(userID, roomInfo, type, contants.defaultCoverImg, 36L,req.getCategory());
    ...
    @Override
    public GetRoomListRsp getRoomListByFollows(String userID, String token, int cnt, int index, int type) {
        GetRoomListRsp rsp = new GetRoomListRsp();
        if (userID == null || token == null) {
            rsp.setCode(2);
            rsp.setMessage("请求失败，缺少参数");
        ...
        rsp.setList(roomMgr.getListByFollows(cnt, index, type, userID));
        return rsp;
    }

    @Override
    public BaseRsp shielding(String userID, String token, List<Long> userIDs, int type) {
        BaseRsp rsp = new BaseRsp();
        ...
        roomMgr.shielding(userIDs, userID, type);
        return rsp;
    }

    @Override
    public GetUserListRsp shieldUsers(String userID, String token, int cnt, int index, int type) {
        ...
        rsp.setList(roomMgr.shieldUsers(cnt, index, userID, type));
        return rsp;
    }

    @Override
    public BaseRsp cancelShielding(String userID, String token, Long shieldingId, int type) {
        BaseRsp rsp = new BaseRsp();
        ...
        roomMgr.cancelShielding(shieldingId, userID, type);
        return rsp;
    }

    @Override
    public GetUserDetailRsp getHostInfo(String userID, String token, String roomID, int type) {
        GetUserDetailRsp rsp = new GetUserDetailRsp();
        if (userID == null || token == null || roomID == null) {
            rsp.setCode(2);
            rsp.setMessage("请求失败，缺少参数");
