$(function() {
	                ...
	                },
	                {
	                	...
	                	"render": function ( data, type, row ) {
	                		return function(){
	                			...
	                			// log url
	                			var logUrl = 'joblog?jobId='+ row.id;
	                			
	                			// log url
	                			var codeBtn = "";
                                if ('BEAN' != row.glueType) {
									var codeUrl = 'jobcode?jobId='+ row.id;
	...
	$('#jobGroup').on('change', function(){
        ...
        window.location.href =  "jobinfo?jobGroup=" + jobGroup;
    });
	...
	var addModalValidate = $("#addModal .form").validate({
        ...
        submitHandler : function(form) {
        	$.post("jobinfo/add",  $("#addModal .form").serialize(), function(data, status) {
    			if (data.code == "200") {
					$('#addModal').modal('hide');
					layer.open({
						title: '系统提示',
	...
	var updateModalValidate = $("#updateModal .form").validate({
        ...
        submitHandler : function(form) {
			// post
    		$.post( "jobinfo/update", $("#updateModal .form").serialize(), function(data, status) {
    			if (data.code == "200") {
